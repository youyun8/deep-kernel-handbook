...
其中 $M$ 是因果遮罩（上三角為 $-\infty$）。兩個 matmul 夾一個 row-softmax。training 時， 我們一次算出全部 $N$ 個 query 位置。
...
這是 inference 最重要的一個優化。但它把瓶頸搬了個位置：現在**每一步都得把整個快取完整 讀一遍**。
...
3. 對於 16-token 區塊，當序列長度在 $[1, 4096]$ 均勻分佈時，最後一個區塊的碎片平均浪費多少 KV 記憶體？
4. MLA 把 K/V 壓成維度 $d_c \ll n_{kv}d_h$ 的 latent。寫出它相對 GQA 的快取大小比，並說明它在 decode 時的計算／記憶體取捨。
