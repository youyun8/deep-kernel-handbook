...
- **Expert offload 到 CPU/NVMe**，按需串流進 HBM。GPU 少很多，但每一層都可能卡住、等它的 expert 經由 PCIe/CXL 抵達。- **量化 expert**（int8/int4/FP8）縮小足跡、好塞進 HBM —— 通常是第一個動的槓桿（見下文）。
...
!!! Example "數值例子：offload 需要多少重用"
    若一個 expert FP4 權重約 2.75 MB，PCIe/CXL 有效頻寬 64 GB/s，載入一次約 $2.75/64{,}000\approx0.043$ ms。若這個 expert 只服務 1 個 token，這 43 μs 直接進入 critical path；若同一批有 32 個 token 打到它，攤到每 token 約 1.3 μs，才可能被 GEMM 或其他 kernel 藏住。offload 是否划算，本質上就是 tokens-per-loaded-expert 的問題。
...
- **Expert 熱門度偏斜**意味著有些 expert 幾乎總是常駐、有些極少用到 —— 可以靠把熱 expert 快取在 HBM、把冷的 offload 出去來利用。- **Prefill 與 decode 差很多**：prefill 有很多 token（大多數 expert 都活躍、批次效果好）；單流 decode 每個 token 只碰 $k$ 個 expert（重用很差） —— 這是把許多並發 decode 請求批在一起的另一 個理由。
!!! Example "數值例子：batch 仍可能碰到幾乎所有 experts"
    對 $B=256$ 筆 decode 請求、$E=256$ 個 experts、$k=8$，總 assignment 數是 $B k=2048$。若先粗略假設 routing 均勻，會被碰到的不同 expert 期望數約為 $E(1-(1-1/E)^{2048})\approx256(1-e^{-8})\approx256$。也就是說，大 batch 幾乎會碰到所有 experts；平均每個 expert 只有 8 個 rows。這解釋了為什麼 MoE serving 同時需要大 batch、grouped GEMM 與熱門 expert 快取。
...
- **Int4（GPTQ/AWQ 風格）**：約小 4 倍；需要仔細校準，但對 expert 通常沒問題 —— 它們單獨來看 不如 attention 或 norm 敏感。- **混合精度**：router、attention、norm 與共享 expert 維持較高精度（它們敏感且小），把許多路由 expert 大力量化。這呼應了 [training 的精度紀律](training-stability.md)：便宜又多的用低精度， 敏感又小的用高精度。
...
- [ ] 用 continuous batching 拉高 tokens-per-expert（攤銷權重讀取 —— 見 [Inference 優化](../performance/inference-optimization.md)）。
...
[2] A. Eliseev and D. Mazur, "Fast inference of mixture-of-experts language models with offloading," *arXiv:2312.17238*, 2023.

[3] E. Frantar and D. Alistarh, "GPTQ: Accurate post-training quantization for generative pre-trained transformers," in *Proc. ICLR*, 2023.
...
[6] Y. Zhong *et al.*, "DistServe: Disaggregating prefill and decoding for goodput-optimized large language model serving," in *Proc. OSDI*, 2024.
